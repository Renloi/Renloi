...
func (c *ServerCommand) DefineFlags() {
	...
	c.FlagMap["data-dir"] = helper.FlagDescriptor{
		Description: fmt.Sprintf("Specifies the data directory used for storing Renloi client data. Default: %s", helper.DefaultConfig().DataDir),
		Arguments: []string{
			"DATA_DIRECTORY",
		},
		FlagOptional: true,
...
func (c *ServerCommand) GetHelperText() string {
	return "The default command that starts the Renloi, by bootstrapping all modules together"
}
