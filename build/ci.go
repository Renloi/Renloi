// Copyright 2016 The renloi Authors
...
import (
	...
	"github.com/renloi/renloi/crypto/signify"
	"github.com/renloi/renloi/internal/build"
...
func doArchive(cmdline []string) {
	var (
		...
		upload  = flag.String("upload", "", `Destination to upload the archives (usually "renloistore/builds")`)
		ext     string
	)
	flag.CommandLine.Parse(cmdline)
	switch *atype {
	...
	*workdir = makeWorkdir(*workdir)
	...
	for _, pkg := range debPackages {
		for distro, goboot := range debDistroGoBoots {
			// Prepare the debian package with the renloi sources.
...
type debMetadata struct {
	...
	// renloi version being built. Note that this
...
func newDebMetadata(distro, goboot, author string, env build.Environment, t time.Time, name string, version string, exes []debExecutable) debMetadata {
	if author == "" {
		...
		author = "Ethereum Builds <fjl@renloi.org>"
	}
	return debMetadata{
...
func doAndroidArchive(cmdline []string) {
	var (
		...
		upload  = flag.String("upload", "", `Destination to upload the archive (usually "renloistore/builds")`)
	)
	flag.CommandLine.Parse(cmdline)
	env := build.Env()
	tc := new(build.GoToolchain)
	...
	build.MustRun(gomobileTool("bind", "-ldflags", "-s -w", "--target", "android", "--javapkg", "org.ethereum", "-v", "github.com/renloi/renloi/mobile"))
...
func doXCodeFramework(cmdline []string) {
	...
	bind := gomobileTool("bind", "-ldflags", "-s -w", "--target", "ios", "-v", "github.com/renloi/renloi/mobile")
...
func doPurge(cmdline []string) {
	var (
		store = flag.String("store", "", `Destination from where to purge archives (usually "renloistore/builds")`)
