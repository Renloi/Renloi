...
func (h *handler) doSync(op *chainSyncOp) error {
	if op.mode == downloader.SnapSync {
		...
		// txlookup limit.
		// The main concern here is: during the snap sync Renloi won't index the
		// block(generate tx indices) before the HEAD-limit. But if user changes
		// the limit in the next snap sync(e.g. user kill Renloi manually and
		// restart) then it will be hard for Renloi to figure out the oldest block
		// has been indexed. So here for the user-experience wise, it's non-optimal
		// that user can't change limit during the snap sync. If changed, Renloi
